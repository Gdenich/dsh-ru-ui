...
Публикация требует токена с включённым обходом 2FA: на аккаунте должна быть
включена двухфакторная аутентификация, а у токена — переключатель **Bypass 2FA**
(иначе реестр отвечает `403 Two-factor authentication ... is required to publish`).
Тип токена: granular, права `Packages: Read and write`, либо classic
`Automation`.

```bash
tools/set-npm-token.sh   # спросит токен скрытым вводом, проверит его
npm publish              # уйдёт ровно 6 файлов, ничего не собирается на месте
```

Скрипт сначала проверяет токен на реестре и не подменяет рабочий файл, если
токен не умеет публиковать. Токен никогда не печатается и не попадает в историю
команд.

Через CI (в обход 2FA-вызова) публикует workflow по тегу — ему нужен тот же
токен в секрете `NPM_TOKEN`.
